...
# 1. RUTA ABSOLUTA DINÁMICA: Mantiene tu lógica de evitar errores de conexión
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, 'lee_conmigo.db')
...
def crear_estudiante(datos_tupla):
    """Inserción con 17 valores: perfil + apellidos + clave_album + clave_estudiante."""
    ...
    query = '''
        INSERT INTO estudiantes (
            ...
            clave_album, clave_estudiante
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    '''
    return ejecutar_query(query, datos_tupla)

def actualizar_estudiante(estudiante_id, datos_tupla):
    """Actualización incluyendo apellidos, clave_album y clave_estudiante (17 campos + id)."""
    query = '''
