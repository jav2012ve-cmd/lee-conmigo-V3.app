...
def render_lecciones_nino():
    ...
    # Cabecera dinámica (solo en vocales; en consonantes no mostramos cintillo superior)
    if es_vocal:
        letras_override = st.session_state.get("v3_letras_override")
        if isinstance(letras_override, list) and letras_override and all(str(x).upper() in ["A", "E", "I", "O", "U"] for x in letras_override):
            # En V3 mostramos el bloque (AEI / IOU / AEIOU) en el título.
            titulo_letras = "-".join([str(x).upper() for x in letras_override])
            st.markdown(
                f"<h1 style='text-align: center;'>Aprendamos las letras {titulo_letras}</h1>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"<h1 style='text-align: center;'>Aprendamos la letra {letra_actual}</h1>",
                unsafe_allow_html=True,
            )
    ...
    if recurso:
        ...
        # --- CONSONANTES: flujo mejorado (sílabas + Vamos a leer + Frases mágicas) o flujo clásico ---
            ...
            if silabas_letra:
                ...
                # Aplicar fondo a la página (mismo estilo que vocales)
                ...
                # Fase: principal (letra, matriz, leo, frases) o actividades:
                # actividad_armar_1, actividad_armar_2, escucha_palabras, escucha_frases
                # Clave por letra + carga desde BD para no perder avance al reiniciar Streamlit.
                k_paso = _key_est(estudiante_id, f"leccion_consonant_paso_{letra_actual}")
                if k_paso not in st.session_state:
                    fase_db = obtener_fase_leccion_consonante(estudiante_id, letra_actual)
                    st.session_state[k_paso] = fase_db if fase_db else "principal"
                elif st.session_state[k_paso] not in FASES_CONSONANTE_VALIDAS:
                    st.session_state[k_paso] = "principal"

                if st.session_state[k_paso] == "principal":
                    # Fila: Motivos | Imprimir lección
                    ...
                    with col_imprimir:
                        ...
                        if PDF_LECCION_DISPONIBLE and ejecutar_job_en_background:
                            ...
                            # Si hay job en curso, consultar estado
                            if job_id:
                            ...
                            if st.session_state.get(k_pdf):
                                ...
                                if st.button("🖨️ Crear Lección para imprimir", use_container_width=True, key="imprimir_leccion"):
                                    try:
                                        ...
                                        palabras_pdf = [{"palabra": (r.get("palabra") or "").strip(), "ruta_img": (r.get("ruta_img") or "").strip()} for r in recursos_pdf if (r.get("palabra") or "").strip()]
                                        frases_pdf = []
                                        ...
                                        for (palabra_clave, _cat, img_path) in album_nino:
                                            ...
                                            if "MAMA" in p or "MAMÁ" in (palabra_clave or "").upper():
                                                ...
                                                break
                                            if nombre_mama_buscar and (nombre_mama_buscar.upper() in p or nombre_mama_buscar in (palabra_clave or "")):
                                                foto_mama_ruta = img_path
                                                break
                                        params = {
                                            "letra": letra_actual,
                                ...
                                with st.spinner("Creando tu hoja…"):
                                    try:
                                        ...
                                        pdf_bytes = generar_pdf_leccion(
                                            letra_actual, silabas_letra, nombre_estudiante=nombre_est,
                                            fondo_ruta=fondo_ruta or "", color_hex=color_fav,
                                            palabras=palabras_pdf, frases=frases_pdf,
                                            foto_estudiante=foto_est or "", foto_mama=foto_mama_ruta or "",
                                        )
