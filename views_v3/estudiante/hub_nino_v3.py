...
from database.db_queries_v3 import categoria_stats_ambas_actividades, stats_actividad_leccion_vocal
...
def render_hub_nino_v3():
    ...
    def _recalcular_album_ciclo(cid):
        try:
            i = next(ii for ii, c in enumerate(CurriculumV3.CICLOS) if c["id"] == cid)
        except StopIteration:
            i = 0
        c_list = CurriculumV3.categorias_habilitadas_para_ciclo_idx(i)
        ec = []
        comp = 0
        for cat in c_list:
    ...
    st.session_state.v3_bloque_lecciones_habilitado = bool(album_ok)
    ...
    _box = (
        ...
        f"background:linear-gradient(180deg,{color_fav}22 0%,#fff 70%);min-height:136px;"
    )
    ...
    with c_welcome:
        st.markdown(
            f"""
            <div style="{_box}">
              <p style="font-size:1.45rem;font-weight:800;color:{color_fav};margin:0;line-height:1.2;">
                ¡Hola, {nombre}!
              </p>
              <p style="font-size:0.92rem;color:#444;margin:0.5rem 0 0 0;">
                V3: camino guiado por ciclos (Álbum → Lecciones).
              </p>
            </div>
            """,
    ...
    with c_actions:
        ...
        with st.container(border=True):
            btn_album = st.button(
                ...
                key="v3_ir_album",
                disabled=bool(album_ok),
                help=(
                    "Álbum del ciclo ya completado; el acceso principal ahora es Lecciones."
                    if album_ok
        ...
        if btn_album:
            ...
            st.rerun()
        if btn_lecciones:
            if ciclo_id == "C1" and "v3_leccion_bloque_idx" not in st.session_state:
        ...
        for e in estado_cats:
            ...
            if primera_pendiente and e["cat"] == primera_pendiente[0] and primera_pendiente[1]:
                ...
                if actividad == "ArmarPalabra":
                    ...
                    if st.button(label_btn, type="primary", use_container_width=True, key=f"v3_btn_go_armar_{e['cat']}"):
                        st.session_state.v3_album_categoria_activa = e["cat"]
                        st.session_state.pagina_activa = "album_nino"
                        st.rerun()
                elif actividad == "EscuchaToca":
                    label_btn = f"Vamos allí (Escucha y Toca: {e['cat']})"
                    if st.button(label_btn, type="primary", use_container_width=True, key=f"v3_btn_go_et_{e['cat']}"):
    ...
    st.write("---")
    if st.button("🏠 Volver al Salón", use_container_width=True, key="v3_volver_salon"):
