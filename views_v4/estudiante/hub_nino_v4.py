...
def render_hub_nino_v4():
    ...
    st.session_state.v4_bloque_lecciones_habilitado = bool(album_ok)
              ...
              <div style="display:grid;grid-template-columns:auto 1fr;gap:0.65rem 0.85rem;align-items:center;">
                ...
                <div style="text-align:left;min-width:11rem;max-width:100%;">
                  ...
                  <p style="font-size:0.88rem;color:#444;margin:0;line-height:1.3;">
                    V4: camino guiado por ciclos (Álbum → Lecciones).
                  </p>
                </div>
              </div>
            </div>
    ...
    with c_actions:
        ...
        if btn_album:
            ...
            st.rerun()
        if btn_lecciones:
            if ciclo_id == "C1" and "v4_leccion_bloque_idx" not in st.session_state:
                st.session_state["v4_leccion_bloque_idx"] = 0
        ...
        for e in estado_cats:
            ...
            if primera_pendiente and e["cat"] == primera_pendiente[0] and primera_pendiente[1]:
                ...
                if actividad == "ArmarPalabra":
                    ...
                    if st.button(label_btn, type="primary", use_container_width=True, key=f"v4_btn_go_armar_{e['cat']}"):
                        st.session_state.v4_album_categoria_activa = e["cat"]
                        st.session_state.pagina_activa = "album_nino"
                        st.rerun()
                elif actividad == "EscuchaToca":
                    label_btn = f"Vamos allí (Escucha y Toca: {e['cat']})"
                    if st.button(label_btn, type="primary", use_container_width=True, key=f"v4_btn_go_et_{e['cat']}"):
    ...
    st.write("---")
    if st.button("🏠 Volver al Salón", use_container_width=True, key="v4_volver_salon"):
