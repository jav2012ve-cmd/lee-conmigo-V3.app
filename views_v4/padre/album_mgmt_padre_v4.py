"""Gestión de álbum 4.0: en Familia (roles predefinidos) solo avatares de galería; otras categorías permiten subir foto."""
import os
...
def render_album_mgmt_v4():
    render_titulo_pagina("Gestión del álbum (4.0)")
    st.write(
        "Modo DEMO activo: solo se gestionan **4 álbumes** (Familia, Juguetes, En la cocina e Instrumentos musicales). "
        "No se permite subir fotos personales; solo selección de avatares de galería."
    )
    ...
    if not acceso_ok:
        ...
        if clave_album_est:
            ...
            clave_ingresada = st.text_input("Clave del álbum", type="password", key="album_clave_input_v4", placeholder="Clave del perfil del niño")
            if st.button("Confirmar acceso", key="album_conf_clave_v4"):
            ...
            pin_ingresado = st.text_input("PIN", type="password", key="album_pin_input_v4", placeholder="Ej: 1234")
            if st.button("Confirmar acceso", key="album_conf_pin_v4"):
            ...
            if st.button("Sí, soy el tutor. Acceder al álbum", key="album_conf_tutor_v4"):
                if estudiante_seleccionado_id not in (st.session_state.album_acceso_confirmado_ids or []):
                    st.session_state.album_acceso_confirmado_ids = list(
                        st.session_state.album_acceso_confirmado_ids or []
                    ) + [estudiante_seleccionado_id]
                st.rerun()
        if st.button("⬅️ Volver al Salón", key="album_volver_salon_gate_v4"):
            st.session_state.pagina_activa = "salon_entrada"
            st.rerun()
        return
    ...
    with st.container(border=True):
        st.markdown("##### 👥 Avatares de familia (4.0)")
        st.caption(
                    ...
                    for row0 in range(0, len(lista_rol), ncols):
                        ...
                        for j, av in enumerate(row_items):
                            ...
                            with gcols[j]:
                                ...
                                if st.button(
                                    "Elegir",
                                    key=f"padre_v4_av_pick_{id_est}_{slug}_{global_idx}",
    ...
    with st.expander("➕ Añadir foto al álbum (deshabilitado en DEMO)", expanded=False):
        st.info(
            "En la DEMO no está permitida la subida de fotos personales. "
            "El niño solo puede representarse con avatares de la galería."
        )
        st.caption(f"Álbumes activos en DEMO: {', '.join(_CATEGORIAS_DEMO)}")
    ...
    if st.button("⬅️ Volver al Salón", key="album_volver_salon_v4"):
        st.session_state.pagina_activa = "salon_entrada"
        st.rerun()
