...
def _mensaje_ruta_v5():
    ...
    """
    flash = st.session_state.pop("v5_flash_msg", None)
    if flash:
        st.success(flash)
        return
    ...
    habilitado = bool(st.session_state.get("v5_bloque_lecciones_habilitado"))
    if habilitado:
...
def render_album_nino_v5():
    est_id = st.session_state.get("estudiante_id")
    if est_id:
        st.session_state.v5_ciclo_id = gamificacion.ciclo_v4_activo(est_id)

    nombre = st.session_state.get("nombre_nino", "Pequeño explorador")
    color_fav = st.session_state.get("color_favorito", "#4A90E2")

    titulo = f"El Álbum de {(nombre or '').strip() or 'Pequeño explorador'} (5.0)"

    def _acciones_v5_album():
        if st.button("Volver al inicio", use_container_width=True, key="v5_album_hdr_hub"):
            st.session_state.pagina_activa = "hub_nino"
            st.rerun()
        if st.button("Abecedario (9×3)", use_container_width=True, key="v5_album_hdr_abecedario"):
    ...
    render_encabezado_logo_titulo_acciones(
        ...
        color_fav=color_fav,
        logo_height=192,
        slot_acciones=_acciones_v5_album,
    )

    _mensaje_ruta_v5()

    st.write("")
    st.markdown("**Categorías del álbum (5.0)**")
    st.caption("Cuatro categorías activas en esta versión; el resto del catálogo se irá incorporando con el programa.")
    ...
    if not categorias_activas:
        st.error("No hay categorías activas reconocidas en el catálogo de álbum.")
        return

    # Solo categorías activas en 5.0: progreso 75% en ambas actividades por categoría.
    estado_cats = []
    ...
    # Primera categoría activa no superada (bloqueo secuencial respecto al orden definido).
    ...
    activa = st.session_state.get("v5_album_categoria_activa")
    if activa not in categorias_activas:
        activa = categorias_activas[0]
        st.session_state.v5_album_categoria_activa = activa
    ...
    for i, e in enumerate(estado_cats):
        ...
        with cols[i % num_cols]:
            if st.button(label, use_container_width=True, key=f"v5_cat_btn_{cat}", disabled=disabled):
                st.session_state.v5_album_categoria_activa = cat
                st.rerun()

    st.write("---")
    categoria_sel = st.session_state.get("v5_album_categoria_activa")
    if not categoria_sel:
        st.info("Elige una categoría para ver tus imágenes.")
    elif categoria_sel not in categorias_activas:
        st.warning("Esta categoría no está disponible en la vista actual.")
    else:
        ...
        cache_key_fam = f"v5_album_items_{categoria_sel}"
                            ...
                            for row0 in range(0, len(lista_rol), ncols):
                                ...
                                for j, av in enumerate(row_items):
                                    ...
                                    with gcols[j]:
                                        ...
                                        if st.button(
                                            "Elegir",
                                            key=f"v5_album_av_pick_{est_id}_{slug}_{global_idx}",
        ...
        if not items:
                ...
                with grid_cols[idx % 3]:
                    if item.get("placeholder"):
                        render_album_card_placeholder(
                            unique_id=f"v5_album_placeholder_{categoria_sel}_{idx}",
                            size=size_card,
                        )
                    else:
                        ...
                        render_album_card_karaoke(
                            item["ruta_img"],
                            nombre_visible,
                            unique_id=f"v5_album_{categoria_sel}_{idx}",
                            size=size_card,
            ...
            if not es_familia or len([i for i in items if not i.get("placeholder")]) >= 2:
                st.write("---")
                if st.button("📝 Jugar: Armar la palabra con sílabas", key="v5_album_btn_silabas", use_container_width=True):
                    st.session_state.album_actividad_categoria = categoria_sel
                    st.session_state.pagina_activa = "album_silabas"
                    st.rerun()

    st.write("---")
    if st.button("⬅️ Volver al Inicio", use_container_width=True, key="v5_album_volver"):
        st.session_state.pagina_activa = "hub_nino"
        st.rerun()
