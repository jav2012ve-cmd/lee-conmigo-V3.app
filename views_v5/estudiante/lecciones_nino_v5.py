        ...
        if cats:
            ...
            for cat in cats:
                ...
                if not ok:
                    habilitado = False
                    break
    st.session_state.v5_bloque_lecciones_habilitado = bool(habilitado)
    if not habilitado:
        st.caption("Completa el álbum del ciclo para desbloquear las lecciones.")
        if st.button("⬅️ Volver a Mi Ruta (5.0)", type="primary", use_container_width=True):
            st.session_state.pagina_activa = "hub_nino"
            st.rerun()
        st.stop()

    # Letras piloto mientras se completa la ruta por ciclos (mismo subconjunto que en 4.0).
    st.markdown("### 🔤 Lecciones (pilotos M y P)")
    st.caption("En 5.0 el abanico de letras crecerá con el programa; por ahora: M y P.")
