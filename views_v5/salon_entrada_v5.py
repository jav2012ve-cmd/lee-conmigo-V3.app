...
def render_salon_entrada_v5():
    # Misma experiencia que main: clave por emojis / «sin clave» sin cintillo 5.0 encima.
    if st.session_state.get("pendiente_confirmar_emoji") is not None:
        _render_salon_entrada_v2()
        return

    st.caption("Versión 5.0: flujo académico guiado por ciclos.")
    ...
    if est_id:
        ...
        st.markdown("### 🪜 Mi escalera de progreso (5.0)")
        completas = 0
        for c in cats:
